...
namespace BradLang.CodeAnalysis.Binding
{
    internal abstract class BoundTreeRewriter
    {
        ...
        {
            var condition = RewriteExpression(node.Condition);
            var trueExpression = RewriteExpression(node.True);
            var falseExpression = RewriteExpression(node.False);

            if (condition == node.Condition &&
                trueExpression == node.True &&
                falseExpression == node.False)
            {
                return node;
            }

            return new BoundTernaryExpression(node.Operator, condition, trueExpression, falseExpression);
        }
